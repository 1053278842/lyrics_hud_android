...
//                        Log.d("LyricHud", (s == PlaybackState.STATE_PLAYING ? "▶ 正在播放" : "⏸ 已暂停") + "，位置: " + pos);
        ...
        public void onMetadataChanged(MediaMetadata metadata) {
            if (metadata != null) {
                ...
                if (!TextUtils.equals(title, lastTitle) || !TextUtils.equals(artist, lastArtist)) {
                    lastTitle = title;
                    lastArtist = artist;
                    Log.d("LyricHud", "🎵 歌曲名: " + title);
                    Log.d("LyricHud", "🎤 歌手: " + artist);
                    Log.d("LyricHud", "💿 专辑: " + metadata.getString(MediaMetadata.METADATA_KEY_ALBUM));
                    Log.d("LyricHud", "💿 专辑作者: " + metadata.getString(MediaMetadata.METADATA_KEY_ALBUM_ARTIST));
                    Log.d("LyricHud", "歌曲时长（ms）: " + duration);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            try {
                ...
                for (MediaController controller : controllers) {
                    if ("com.kugou.android".equals(controller.getPackageName())) {
                        if (kugouController == null || !kugouController.getSessionToken().equals(controller.getSessionToken())) {
                            ...
                            Log.d("LyricHud", "🔁 MediaSession 变更，重新绑定回调");
                            triggerCallbacks();
                        }
                        return;  // 找到了，提前返回
                    }
                ...
                if (kugouController != null) {
                    ...
                    Log.d("LyricHud", "🛑 酷狗已关闭，清空 MediaController");
                }
            } catch (SecurityException e) {
                Log.e("LyricHud", "❌ 获取 MediaSession 权限失败：请确保已授予通知访问权限", e);
            }
        }
    }
                ...
                if (state != null) {
                    ...
                    if (customActions != null && !customActions.isEmpty()) {
                        for (PlaybackState.CustomAction action : customActions) {
                            Log.d("LyricHud", "CustomAction id=" + action.getAction()
                                    + ", name=" + action.getName()
                                    + ", icon=" + action.getIcon()
                            ...
                            if (extras != null && !extras.isEmpty()) {
                                ...
                                Log.d("LyricHud", "Extras 包含 " + keys.size() + " 项");
                                for (String key : keys) {
                                    Object value = extras.get(key);   // 根据键取值
                                    Log.d("LyricHud", "Extra Key=\"" + key + "\", Value=\"" + value + "\"");
                                }
                            } else {
                                Log.d("LyricHud", "CustomAction extras 为空");
                            }
                        }
                    } else {
                        Log.d("LyricHud", "No custom actions in playback state");
                    }
