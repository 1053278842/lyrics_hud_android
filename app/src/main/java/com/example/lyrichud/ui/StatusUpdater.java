...
public class StatusUpdater {
    ...
    public void update() {
        ...
        LinearLayout hotspotItem = (LinearLayout) statusNotice.getChildAt(1);
        LinearLayout bleItem = (LinearLayout) statusNotice.getChildAt(2);
        LinearLayout peripheralItem = (LinearLayout) statusNotice.getChildAt(3);
        LinearLayout musicBarItem = (LinearLayout) statusNotice.getChildAt(4);
    ...
    private void updateItem(LinearLayout item, int textResId, boolean status) {
        ...
        // 根据状态更新图标和文本
        icon.setImageResource(status ? R.drawable.ic_verified : R.drawable.ic_error); // 假设你有 ic_verified 和 ic_error 图标
        text.setText(textResId);
//        text.setTextColor(status ? activity.getColor(R.color.colorAccent) : activity.getColor(R.color.colorFail));
